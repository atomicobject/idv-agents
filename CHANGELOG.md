...
- **Marketplace install failure on v1.2.0** — v1.2.0 was the first release to commit `dist/` and its install zips to the repo, which broke the Cowork marketplace installer. `dist/` is now gitignored; the install zips live only as GitHub release assets (where the README has always pointed users for the fallback path). Skill content is unchanged from v1.2.0.
...
[1.2.1]: https://github.com/atomicobject/idv-agents/releases/tag/v1.2.1
